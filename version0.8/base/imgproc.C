...
//===============================turntable=============================================
void ImgProc::renderWedges()
{
    struct Noise_t mynoise;
    mynoise.frequency = 0.666666;//
    //noise_a.falloff = 0.8;
    mynoise.octaves = 1.0;//
    mynoise.roughness = 1.0;
    mynoise.gamma = 2.0;//
    mynoise.amplitude = 3;
    mynoise.fjump = 2.5;//around 2
    mynoise.falloff = 0.4;//0.9,0.8,0.7,0.6,0.4

    float fre[7]={0.3,0.6,1.0,2.0, 3.5,6.0,8.0};
    float oct[3]={1.0, 2.5, 5.0};
    float fj[5]={1.6,1.8,2.0,2.6,3.0};
    for(int i=0;i<7;i++)
    {
      mynoise.frequency=fre[i];
      for(int j=0;j<3;j++)
      {
        mynoise.octaves=oct[j];
        for(int k=0;k<5;k++)
        {
          mynoise.fjump=fj[k];
          render(mynoise);
...
//==================================================================================
//volume rendering
void ImgProc::render(Noise_t mynoise)//input: density field, color field
{
    ...
    //render noise
    Grids *gr_obj = new Grids(Vector(-5, -5, 5), 251,251,251,0.04);

    cout<<"======test1111111"<<endl;
    NoiseWedge *obj_nw = new NoiseWedge(*gr_obj, Vector(0.0, 0.0, 2.0), 3.0, mynoise);
    //PyroclasticWedge *obj_pw = new PyroclasticWedge(*gr_obj, 2.0);
    //obj_pw->Setnoise(mynoise);
    //obj_pw->eval();
    ...
    GridsToFields *gtf_obj = new GridsToFields(obj_nw->get_grid());
    ...
    Color noise_color(1.0d,1.0d,1.0d,0.0d);

    //int test = 0;

    vector<GridsToLightFields*> light_fields;
    int light_num = 3;
    Color c_light(0.4d,0.6d,0.8d,0.0d);
    ...
    for(int k=0;k<light_num;k++)
    {
      ...
      Grids *gr_light = new Grids( Vector(-5.0, -5.0, 5.0), 201,201,201,0.05 );
...
//material = (1, 1, 1)
          ...
          Lp = Lp + noise_color * Tl * c_light * (1.0 - del_t) / K * T;
...
void ImgProc::render()//input: density field, color field
{
    ...
    //WispsWedge *obj_ww = new WispsWedge(*gr_obj, 2.0, Vector(-3, -3, 0), 0.0001);
    // WispsWedge *obj_ww = new WispsWedge(*gr_obj, 5.0, Vector(-0, -0, 0), 0.01);
    GridsToFields *gtf_obj = new GridsToFields(obj_pw->get_grid());
    // cout<<"======test3333333"<<endl;
    ...
    for(int k=0;k<light_num;k++)
    {
      ...
      Grids *gr_light = new Grids( Vector(-6.0, -6.0, 6.0), 121,121,121,0.1 );
      DSM_Grids *dsm_grids = new DSM_Grids(*gr_light, l, gtf_obj);
      //DSM_Grids2 *dsm_grids = new DSM_Grids2(*gr_light, l, teapot_avator_den);
      dsm_grids->bakeDSM();
      GridsToLightFields *gtf_light = new GridsToLightFields(dsm_grids->get_grid());
