...
//Build Terrain
class Terrain : public Volume<float>
{
  public:

    Terrain()
    {
      ObjectPlane *terrain_ground = new ObjectPlane(Vector(0.0, 0.0, 0.0), Vector(0.0, 1.0, 0.0));
      struct Noise_t noise_a;
      noise_a.frequency = 0.2;
      noise_a.fjump = 2.0;
      noise_a.falloff = 0.8;//0.8;
      noise_a.gamma = 1.0;//0.8;
      noise_a.octaves = 3.0;//3.0;
      noise_a.amplitude = 10;
      FSPN *fspn = new FSPN(noise_a);
      CPT *cpt = new CPT(terrain_ground);
      Warp *PlaneWarp = new Warp(fspn, cpt);
      pyro_ground= new PyroDisplacement(terrain_ground, PlaneWarp);
    ...
    const float eval(const Vector& x)const
    {
      ...
      // //float fspn = fs->eval(x)
      // //FSPN *fspn = new FSPN(noise_a);
      //
      // Vector cpt = x - terrain_ground->eval(x) * terrain_ground->grad(x);
      // //CPT *cpt = new CPT(terrain_ground);
      //
      // float fspn = fs->eval(cpt);
      // //Warp *PlaneWarp = new Warp(fspn, cpt);
      //
      // if(fspn<0)
      // {
      //   float result = -1.0*pow((fspn*-1.0), 2.0)*10;
      //   //cout<<"result: "<<result<<endl;
      //   return terrain_ground->eval(x)+result;
      // }
      // else
      // {
      //   float result = pow(fspn, 30.0)*30;
      //   //cout<<"result: "<<result<<endl;
      //   return terrain_ground->eval(x)+result;
      // }
      // //PyroDisplacement *pyro_ground= new PyroDisplacement(terrain_ground, PlaneWarp);
      // //float pyro = pyro_ground->eval(vv);//x
      // //cout<<"pyro: "<<pyro<<endl;
      // //return pyro;
      ...
      // Warp *PlaneWarp = new Warp(fspn, cpt);
      // PyroDisplacement *pyro_ground= new PyroDisplacement(terrain_ground, PlaneWarp);
      // float pyro = pyro_ground->eval(x);//vv
      // // cout<<"pyro: "<<pyro<<endl;
      // // do{cout<<'\n'<<"press any key to continue";}while(cin.get()!='\n');
      // return pyro;
