...
//======================================transformation========================================================
	...
	public:
		...
		ObjectColorDensityFieldMultiple(GridsToFields* g, Volume<Color>* c) : g(g), color(c) {eval2 = true;}
	...
	class ObjectDensityFieldAdd2 : public Volume<float>//return type is whatever in the <>
	{

	public:

		ObjectDensityFieldAdd2(GridsToFields* f, GridsToFields* d) : elem(f), dens(d) {}
		ObjectDensityFieldAdd2(ObjectDensityFieldAdd2* f, GridsToFields* d) : odf(f), dens(d) {eval2 = true;}
		~ObjectDensityFieldAdd2() {};

		const float eval(const Vector& x)const
		{
			if(eval2)
				return (odf->eval(x) + dens->eval(x));
			else
				return (elem->eval(x) + dens->eval(x));
		}
