...
class NoiseWedge
{
  public:
    ...
    NoiseWedge(Grids G, Vector O, float r) :
       g (G),
       radius (r)
    {
      //float s = g.get_delta();
      ...
      for(int z=0; z< g.Z(); z++)
      {
          for(int y = 0; y < g.Y(); y++)
          {
            for(int x = 0; x< g.X(); x++)
            {
              ...
              if(distance<r)
              {
                if(distance<noise_a.falloff*r)
                  g.get_data()[n] = max((float)0.0, fs->eval(p));// + 1.0 - distance;
                else
                  g.get_data()[n] = max((float)0.0, fs->eval(p)) * (1-distance/r)/(1-noise_a.falloff);
      ...
      for(int z=0; z< g.Z(); z++)
      {
          for(int y = 0; y < g.Y(); y++)
          {
            for(int x = 0; x< g.X(); x++)
            {
              Vector p = g.pos(x, y, z);
              float n = g.gridindex(x, y, z);
              float temp = R - p.magnitude() + pow(abs(fs->eval(p.unitvector() * R)),  noise_a.gamma);
              if(temp < 0)
                temp = 0;
              else
                temp = 0.5;
...
class WispsWedge
{
  public:
    ...
    WispsWedge(Grids G,  float pscale, Vector O, float rho, Noise_t noise) ://grids, radius, center of sphere
       g (G),
       pscale (pscale),
       rho(rho)
    {
