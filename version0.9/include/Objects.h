...
namespace lux
{
	...
	public:
		...
		const Vector grad( const Vector& P ) const
		{
			...
			float delta = 0.01;
			float fxp = eval(P+Vector(delta, 0, 0));
			float fxm = eval(P-Vector(delta, 0, 0));
			float gx = (fxp-fxm)/(2*delta);
			float fyp = eval(P+Vector(0, delta, 0));
			float fym = eval(P-Vector(0, delta, 0));
			float gy = (fyp-fym)/(2*delta);
			float fzp = eval(P+Vector(0, 0, delta));
			float fzm = eval(P-Vector(0, 0, delta));
			float gz = (fzp-fzm)/(2*delta);
