...
{
	std::cout<<"Perlin: X:"<<vec.X()<<"Y: "<<vec.Y()<<"Z: "<<vec.Z()<<std::endl;
...
#define at3(rx,ry,rz) ( rx * g3[i][0] + ry * g3[i][1] + rz * g3[i][2] )
	...
	std::cout<<"Perlin: "<<lerp(sz, c, d)<<std::endl;
	return lerp(sz, c, d);
}
...
void PerlinNoise::init(void)
{
	...
	for (i = 0 ; i < B_PERLIN ; i++) {
		p[i] = i;

		g1[i] = (float)((rand() % (B_PERLIN + B_PERLIN)) - B_PERLIN) / B_PERLIN;//random

		for (j = 0 ; j < 2 ; j++)
			g2[i][j] = (float)((rand() % (B_PERLIN + B_PERLIN)) - B_PERLIN) / B_PERLIN;//random
		normalize2(g2[i]);

		for (j = 0 ; j < 3 ; j++)
			g3[i][j] = (float)((rand() % (B_PERLIN + B_PERLIN)) - B_PERLIN) / B_PERLIN;//random
		normalize3(g3[i]);
	}

	while (--i) {
		k = p[i];
		p[i] = p[j = rand() % B_PERLIN];//random
		p[j] = k;
	}

	for (i = 0 ; i < B_PERLIN + 2 ; i++) {
		p[B_PERLIN + i] = p[i];
		g1[B_PERLIN + i] = g1[i];
		for (j = 0 ; j < 2 ; j++)
			g2[B_PERLIN + i][j] = g2[i][j];
		for (j = 0 ; j < 3 ; j++)
			g3[B_PERLIN + i][j] = g3[i][j];
	}
}
