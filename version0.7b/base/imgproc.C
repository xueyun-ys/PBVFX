...
void ImgProc::render(Camera cam)//input: density field, color field
{
    ...
    //obj.getfield()->eval()???????
    //Grids gr_avator(Vector(-10.0, -10.0, -10.0), 201,201,201,0.1);
    //Grids *gr_avator = new Grids(Vector(-6.0, -6.0, 6.0), 131,131,131,0.1);
    ...
    //Grids *gr_lvlset = new Grids(Vector(-20.0, -20.0, 20.0), 401,401,401,0.1);
    ...
    int light_num = 3;
    //Color c_light(0.5d,0.3d,0.3d,0.0d);
    Color c_light(1.0d,1.0d,1.0d,0.0d);

    vector<Vector> positions;
    Vector pos_light(0.0,8.0,8.0);//Vector pos_light(0.0,8.0,6.0);Vector pos_light(0.0,-4.0,8.0);
    positions.push_back(pos_light);
    Vector pos_light2(0.0,0.0,-8.0);//Vector pos_light2(0.0,0.0,-8.0);Vector pos_light2(0.0,0.0,-8.0);
    positions.push_back(pos_light2);
    Vector pos_light3(0.0,-8.0,0.0);//Vector pos_light3(0.0,-8.0,0.0);Vector pos_light3(0.0,-15.0,0.0);
    ...
    for(int k=0;k<light_num;k++)
    {
      ...
      //Grids *gr_light = new Grids(Vector(-10.0, -10.0, 10.0), 401,401,401,0.05);
...
//cout<<"gtf: "<<gtf_obj->eval(X)<<endl;;
          ...
          del_t = exp(-1.0 * K * del_s * teapot_avator_den->eval(X));
          float Tl = 0.0;
...
//material = (1, 1, 1)
          ...
          Lp = Lp + teapot_avator_color->eval(X) * Tl * c_light * (1.0 - del_t) / K * T;//c_f_bunny->eval(X)
          //Lp = Lp + levelset_bunnycolor *(1.0 - del_t) / K * T;  //wrong
          //Lp = Lp + levelset_bunnycolor * Tl * c_light * (1.0 - del_t) / K * T;
