...
void PerlinNoise::init(void)
{
	...
	for (i = 0 ; i < B_PERLIN ; i++) {
		...
		g1[i] = (float)((random() % (B_PERLIN + B_PERLIN)) - B_PERLIN) / B_PERLIN;

		for (j = 0 ; j < 2 ; j++)
			g2[i][j] = (float)((random() % (B_PERLIN + B_PERLIN)) - B_PERLIN) / B_PERLIN;
		normalize2(g2[i]);

		for (j = 0 ; j < 3 ; j++)
			g3[i][j] = (float)((random() % (B_PERLIN + B_PERLIN)) - B_PERLIN) / B_PERLIN;
		normalize3(g3[i]);
	}

	while (--i) {
		k = p[i];
		p[i] = p[j = random() % B_PERLIN];
		p[j] = k;
	}
